...
VOC_ROOT = os.path.join(HOME, "object_detection/data/VOCdevkit/")
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes, device):
    ...
    def forward(self, x):
        ...
        if self.phase == "test":
            output = self.detect(
                loc.view(loc.size(0), -1, 4).to(self.device),  # loc preds
                self.softmax(conf.view(conf.size(0), -1,
                                       self.num_classes)).to(self.device),  # conf preds
...
def multi_box(vgg_result, extra_layers, cfg, num_classes):
    loc_layers = []
    conf_layers = []
    vgg_source = [21, -2]
    for k, v in enumerate(vgg_source):
        loc_layers += [nn.Conv2d(vgg_result[v].out_channels,
                                 cfg[k] * 4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(vgg_result[v].out_channels,
    ...
    return vgg_result, extra_layers, (loc_layers, conf_layers)
...
class ObjectDetector(Pretrained):
    ...
    def _build_ssd(self, phase, size=300, num_classes=21):
        ...
        base_, extras_, head_ = multi_box(vgg(base[str(size)], 3),
